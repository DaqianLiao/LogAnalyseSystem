...
Vsim_service_availability = r'_simo_nw_info_trace_check.*service_availability = 3'
Vsim_dn_status = r'_simo_nw_info_trace_check.*dn_status = 7'
Vsim_data_success = r'simo_local_rev_channel_payload.*port = \d+'
exception_id = r'exception_id = 0x\d+[a-z]'
...
##    print('{%s}total line:%d'%(file_path,len(lines)))
    ...
    list_map3 = publicfunction.PublicProcessFunction.get_function_Unit(lines, date_time, ticks, Vsim_dn_status)
    list_map4 = publicfunction.PublicProcessFunction.get_function_Unit(lines, date_time, ticks, Vsim_data_success)
    list_map5 = publicfunction.PublicProcessFunction.get_function_Unit(lines, date_time, ticks, exception_id)
    list_map6 = publicfunction.PublicProcessFunction.get_function_Unit(lines, date_time, ticks, PushRemalloc)
