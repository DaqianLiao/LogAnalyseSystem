...
ticks = r'ticks:(\d+)'
cause_value = r'cause value = 21'
exception_id = r'exception_id = 0x02000029'
...
##    print('{%s}total line:%d'%(file_path,len(lines)))
    ...
    list_map3 = publicfunction.PublicProcessFunction.get_function_Unit(lines, date_time, ticks, cause_value)
...
def  analys_txtfile_all(log_path):    
    for dirpath, dirnames, filenames in os.walk(log_path):
        ...
        if TxtList:
           ...
           sStr1 = 'remalloc_no_subscription_analyze_' + time_str + '.xls'
...
def no_subscription_main(log_path):
